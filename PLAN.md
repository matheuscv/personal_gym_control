...
- **11/08/2026** — Refinamento visual v2, a partir de feedback de uso real: tipografia de título trocada de Oswald (caixa-alta condensada) para Manrope, sem forçar caixa-alta em `h1` (só em `.section-title`). Estados vazios de Treino do dia/Evolução Treino/Evolução Corporal reformulados com componente `EmptyState` reutilizável (ícone `lucide-react`, texto convidativo, botão de ação real) no lugar de dois parágrafos soltos. Marca do app no topo ganhou ícone e peso visual. Tema agora tem alternância manual claro/escuro (`ThemeProvider` + `localStorage` + script anti-flash no `index.html`), além de continuar respeitando o tema do sistema por padrão. E-mail + botão "Sair" substituídos por um menu suspenso (`UserMenu`) com 4 opções: Minha conta (editar nome e trocar senha, rota `/conta`, disponível também no Android), Configuração (a antiga tela Admin, renomeada e tirada da navegação principal), Convidar alguém (gera link para `/login?convite=` com e-mail pré-preenchido — sem envio real de e-mail por ora, pois a conta não tem domínio verificado no Resend para isso), e Sair. Bug real corrigido durante a verificação: o botão de mostrar/ocultar senha em `/conta` herdava o estilo do botão de submit por um seletor CSS genérico demais.
- **11/08/2026** — Reestilização visual completa do app + navegação global. Tema escuro "industrial de academia" (grafite/âmbar, Oswald+Inter+JetBrains Mono) virou o padrão via `prefers-color-scheme`, mantendo uma variante clara equivalente (sem toggle manual, não pedido). Novo `AppShell` com topbar + abas-pílula fixas em toda rota autenticada, resolvendo estruturalmente a falta de navegação de volta para Home. Home reescrita como tela de boas-vindas (saudação, plano ativo, atalhos, dicas de saúde/exercício) — "Treino do dia" virou item de menu próprio (`/treino`) em vez de ser a própria Home. Bug real corrigido durante a verificação: sub-nav do Admin marcava "Relatório Corporal" e "Importar Relatório" como ativos simultaneamente (`NavLink` sem `end` casando com o path aninhado). Verificado: build web e Android (bundle admin continua fora do Android), tema claro e escuro, e fluxo funcional completo (marcar série, navegar entre todas as abas).
